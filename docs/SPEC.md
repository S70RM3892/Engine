...
| 17 | エンジン特性を活かしたインクリメンタルゲーム (別アプリ扱い・演出増し) | **ENGINE EMPIRE**: 別アプリ (別パッケージ・別アイコン) として配布。シミュレータの物理をそのまま「発電所」として使い、実出力・熱効率・回転域がゲーム性になる。炎/火花/煙/閃光/シェイク等の演出を追加 (§10) |
...
シミュレータとは **別アプリ** (Gradle productFlavor `empire`、パッケージ `com.s70rm3892.engineempire`、アプリ名・アイコン・セーブデータも別)。ネイティブコアは共通 (`app/src/main`)。3D ビューは全面表示で、ゲーム演出と自動周回カメラを有効にする。
...
| 自動ダイナモ | `x = (rpm − idle)/(0.9·red − idle)`、`load = 0.9·x^1.6` (x ≤ 1)、`0.9 + 6(x − 1)` (x > 1, 上限 3) → 全開で約 0.8〜0.96×レッドラインで釣り合い、蒸気機関など調速機の無い機関も過回転しない |
| スイートゾーン | ガソリン 0.72〜0.95 / ディーゼル 0.62〜0.92 / 電動機 0.30〜0.62 / タービン 0.88〜1.06 / プロペラ 0.78〜1.0 / 蒸気 0.80〜1.35 / スターリング 0.80〜1.10 (レッドライン比) |
...
- **エンジンツリー (スキルツリー型)**: 組み込みカタログ 43 機種 + ルート (耕運機 単気筒) の 44 ノードを 11 系統 (外燃 / 単気筒 / 直列 / 過給 / ディーゼル / 電動 / ロータリー / 水平対向 / V 型 / 星型 / タービン) に配置。**どの系統から伸ばすかはプレイヤーが決める**。ノードは「親のどれか 1 台」を所持していれば解放でき、系統をまたぐ合流点 (例: V8 スーパーチャージ ← 直 4 ターボ or V8 クロス、ターボジェット ← 複列 18 or Jumo 205、誘導モータ ← 直列 3 / V4 / 水平対向 2) がある。
  - 価格 = 無改造・全開での純収入目安 × 60 秒 × 1.5^深さ (親より安くならないよう補正)。画面はドラッグでパン、タップで詳細 (系統・目安収入・必要な親・カタログの解説) と解放/乗り換え。
  - 1 台所持ごとに全収入 +10% (コレクション・ボーナス)。旧版 (一本道) のセーブは新しいキーへ移行する。

![エンジンツリー](images/game_tree.png)
...
HUD の「内部を見る」で、シミュレータと同じ描画モード (外観 / 透視 = X-RAY / 断面 = SECTION / 温度 = THERMAL / 応力 = STRESS) と視点プリセット (全体 / 断面 / クランク / バルブ / 出力軸) を切り替えられる。**スロー ×1/16** は映像だけを減速 (収入・音・物理は実時間) してピストンやバルブの動きを追えるようにし、自動周回カメラは ON/OFF できる。開いている間は計器を 1 行表示にしてエンジンを見やすくする。

### 10.4 演出 (`RenderFrontend::setEffects(1)`)
...
- ツリーの整合性 (キー重複なし・親の存在・深さ・価格・配置の重なりなし・カタログ全機種の収録)、分岐の自由度、旧セーブの移行もテストする。
- `tests/game_balance.cpp`: ツリーの全 44 エンジンを自動ダイナモで定常運転し、出力・燃料・純収入・回転域を表示 (全開で黒字であることを CI で確認)。全アップグレード最大の JSON も同じツールで読み込み安定性を確認。
