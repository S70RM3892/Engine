...
- **サウンド**: 物理音響合成 + 聴感補正 (モスキート域の除去・純音の上限・低域シェルフ・仮想低音・バスコンプレッサ)。`tests/audio_check` で全機種のスペクトルを確認
- **ENGINE EMPIRE**: 同じ物理エンジンで動く育成ゲーム (**別アプリ** `com.s70rm3892.engineempire`)。昼はシフト (動くターゲット帯に回転を合わせてコンボ・ニトロ・金のボルト・ノッキング・エンジン特性から作られる依頼・熱と耐久・日替わりイベント)、夜はガレージ (強化ボード・チューン・仕様・エンジンツリー・目標) のサイクルで、限られた ¥ と ★ の使い道を選ぶ

![ENGINE EMPIRE: シフト中の HUD (依頼・タイマー・耐久)](docs/images/game_hud.png)

![ENGINE EMPIRE: 夜のガレージ (強化ボード)](docs/images/game_night.png)
